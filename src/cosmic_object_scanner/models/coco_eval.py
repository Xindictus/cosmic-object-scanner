...
class CocoEvaluator:
    def __init__(self, coco_gt: Any, iou_types: list[str] | tuple[str, ...]) -> None:
    ...
    def synchronize_between_processes(self) -> None:
        for iou_type in self.iou_types:
            self.eval_imgs[iou_type] = np.concatenate(self.eval_imgs[iou_type], 2)  # type: ignore[assignment]
...
def merge(img_ids: Any, eval_imgs: Any) -> tuple[np.ndarray[Any, Any], np.ndarray[Any, Any]]:
    all_img_ids = utils.all_gather(img_ids)
    all_eval_imgs = utils.all_gather(eval_imgs)

    merged_img_ids: list[Any] = []
    for p in all_img_ids:
        merged_img_ids.extend(p)

    merged_eval_imgs: list[Any] = []
...
def evaluate(imgs: Any) -> tuple[Any, np.ndarray[Any, Any]]:
    with redirect_stdout(io.StringIO()):
        imgs.evaluate()
