    ...
    for prediction in predictions:
        ...
        for box in boxes:
            ...
            for idx, gt in enumerate(ground_truth):
                ...
                if (
                    iou >= iou_threshold
                    and pred_class == gt_class
                    and idx not in matched_annotations
                ):
                    true_positives += 1
                    pred_matched = True
...
def parse_yolo_label(label_file: str, img_width: int, img_height: int) -> list[dict[str, Any]]:
    ...
    for line in lines:
        ...
        annotations.append({"class_id": int(class_id), "bbox": [xmin, ymin, xmax, ymax]})
    return annotations
...
def main(args: Any) -> None:
    ...
    cm = create_confusion_matrix(ground_truths, predictions, NUM_CLASSES + 1)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-m", "--model", type=str, required=True, help="Path to the pretrained YOLO model"
    )
    parser.add_argument(
        "-ti",
        "--test-img",
